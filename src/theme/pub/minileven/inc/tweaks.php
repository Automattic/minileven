...
function minileven_continue_reading_link() {
	return ' &hellip; <a href="'. esc_url( get_permalink() ) . '">' . __( 'Continue reading <span class="meta-nav">&rarr;</span>', 'minileven' ) . '</a>';
}
...
function minileven_wp_title( $title, $sep ) {
	...
	if ( $paged >= 2 || $page >= 2 )
		$title .= " $sep " . sprintf( __( 'Page %s', 'minileven' ), max( $paged, $page ) );
